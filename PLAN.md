...
- **Published to GitHub Pages.** `main` pushed to `github.com/ninad-k/trading-playbook` with `docs/`
  committed; Pages serves it at `https://ninad-k.github.io/trading-playbook/`.
- **Added figures.** 13 original schematic SVGs (`scripts/16_make_diagrams.py`), inlined on all 13
  topic pages and 23 book notes, plus data charts on the index and coverage pages generated from the
  live counts. All theme-aware, all self-contained, none traced from a source.
- **Made the site self-explanatory.** Added an About page covering what the library is, how a note is
  made and what every label means; decoded the bare A/B tier badges with tooltips and a homepage
  legend; collapsed the 733-item A-Z and systems catalogues that were dumped inline on the homepage;
