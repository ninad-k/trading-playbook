...
4. **Method I — Volatility Breakout** (built on The Squeeze): wait for a Squeeze (BandWidth at a 6-month low), then buy a close above the upper band / sell short a close below the lower band; exit via a Parabolic stop or a tag of the opposite band; standard 20-day/2σ bands, or tighten to 15-day/1.5σ for shorter-term traders; watch for head fakes and use a volume indicator (Intraday Intensity, Accumulation/Distribution, or Money Flow Index) to lean toward the likely breakout direction before it confirms.
5. **Method II — Trend Following**: buy when %b > 0.8 **and** 10-period MFI > 80 (both price and volume-weighted momentum are strong); sell/short the mirror condition, %b < 0.2 and MFI < 20; indicator length should run about half the length of the band's calculation period; exit via Parabolic or opposite-band tag; a pullback-entry variant (buy the first up day after the signal rather than the signal bar itself) trades fewer setups but improves risk/reward.
6. **Method III — Reversals**: buy when %b < 0.05 **and** a volume oscillator (21-day Intraday Intensity % or 20-day Accumulation/Distribution %) is positive (a lower-band tag *not* confirmed by weak volume); sell/short when %b > 0.95 and the oscillator is negative; a fuller pattern version looks for a W-bottom where %b is higher on the second low than the first (a "relative W," confirmed if the volume oscillator shows the same higher-low pattern), or the mirror M-top with three or more declining pushes.
7. **Pattern rule for W/M**: an ideal W has its first low outside the lower band and its second low inside the lower band (even if the second low is a new absolute low); the mirror applies to M-tops, which typically take three or more pushes rather than two.
