...
The book documents six complete turnkey systems in Chapter 6, each with its own authoritative rule set as a system sub-page in this library: [[george-pruitt-building-winning-trading-systems-with-tradestation--king-keltner]], [[george-pruitt-building-winning-trading-systems-with-tradestation--bollinger-bandit]], [[george-pruitt-building-winning-trading-systems-with-tradestation--thermostat]], [[george-pruitt-building-winning-trading-systems-with-tradestation--dynamic-break-out-ii]], [[george-pruitt-building-winning-trading-systems-with-tradestation--super-combo]], and [[george-pruitt-building-winning-trading-systems-with-tradestation--ghost-trader]]. Two book-wide rules apply across all of them:
...
A seventh construct, the **Money Manager** template (Chapter 6), is not a trading system but a position-sizing overlay: `numContracts = (initCapital × riskPercent) / marketRisk`, where `marketRisk = StdDev(Close,30) × BigPointValue`, rounded down to a whole contract. It demonstrates volatility-normalized bet sizing (a market with higher current volatility gets fewer contracts for the same dollar risk) rather than a fixed contract count.
...
- [[dynamic-breakout-ii-strategy]] — a shorter Tier B excerpt of the same system; this book's sub-page is the fuller version with complete pseudocode and source.
- [[king-keltner-trading-strategy]] — likewise, an existing shorter excerpt this book covers in full.
- [[ghost-trader-trading-strategy]] — the same relationship for the Ghost Trader trade-filtering template.
- [[van-tharp-trading-systems]] — complements this book's system-construction focus with the business-plan and position-sizing side Pruitt and Hill leave to a single template.
- [[curtis-faith-way-of-the-turtle]] — another fully disclosed, rules-based trend-following system with multi-market backtesting, useful for comparison.
