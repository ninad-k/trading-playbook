...
CTOD is an intraday opening-range breakout system: it defines a trading range from the first two hours of the session using 5-minute closing (not high/low) prices, then trades the break of that range in either direction, confirmed on a 5-minute closing basis rather than an intraday tick touch. It is designed for active, liquid markets — Bernstein names currencies, stock indices, and Treasury bonds as good fits — and is presented as Bernstein's own earlier alternative to the newer breakout methods he developed later (see [[bernstein-jake-the-compleat-day-trader-vol-ii]] for the 30-Minute Breakout).
...
Risk per trade is bounded by the distance from entry to the opposite-side 5-minute closing stop established in step 4, which is set by the width of the first two hours' closing range rather than a fixed dollar or percent figure — a wider morning range produces a wider initial risk. No percent-of-equity position-sizing rule is given; the trader is expected to size the position so that the range-derived stop distance corresponds to an acceptable dollar loss. The hourly-ratcheting trailing stop (step 5) is the system's primary mechanism for protecting profit once the trade is working, rather than a separate profit target.

## Caveats

The specific 2-hour window is a default, not a fixed requirement — Bernstein explicitly invites testing 1-hour or 1.5-hour variants, so the "2 hours" figure should be read as a starting parameter rather than an optimized constant. No win rate, average trade, or drawdown statistic for CTOD specifically survives in the sampled OCR pages (unlike some of the book's other systems, which do include historical performance tables); its support here is the rule description and qualitative tips rather than a backtest. The system assumes an actively-traded instrument with meaningful intraday range in the first two hours — Bernstein notes it works best in currencies, stock indices, and Treasury bonds, and cautions that currently inactive markets could become viable for CTOD (or vice versa) as trading activity shifts over time.
