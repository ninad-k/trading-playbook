...
The complete original Turtle System (N, System 1/System 2 breakout entries, unit sizing, 2N stops, pyramiding, and exits) is documented in the companion system page [[curtis-faith-way-of-the-turtle--turtle-system]]; the numbers there match the book's own reprint of the "Original Turtle Trading Rules." In the main chapters Faith also outlines the four lessons he says summarize the whole program: trade with an edge, manage risk, be consistent, and keep it simple. He additionally sketches several non-Turtle systems built to teach the same concepts with different entries: an ATR Channel Breakout (enter on a close outside a volatility-scaled channel around a moving average), a Bollinger Breakout (enter on a close outside a standard-deviation band), a Donchian Trend system (20-day breakout entries filtered so only trades in the direction of a longer-term trend are taken, with a 10-day opposite breakout exit), and a time-exit variant that closes trades after a fixed holding period instead of waiting for a reverse breakout. These are presented as illustrations of system-building principles rather than as a second complete trading plan, and the book does not give every parameter needed to reproduce them exactly.
...
Faith's central psychological story is the Turtles' first big trend, a February 1984 heating oil breakout: trained identically, he was the only Turtle who took and held the full position through a scary pullback, and ended up making roughly three times what his better-credentialed classmates made — a result he attributes entirely to emotional discipline, not knowledge. He frames trading as a "losers' game" psychologically: most trades lose, so a trader must be comfortable being wrong often and must not need each individual trade to work in order to feel competent. He also warns against curve-fitting and "system death" anxiety — mistaking a normal drawdown for a broken system — and argues that traders who abandon a valid system after a string of losses, rather than after honest testing shows it has stopped working, are making a psychological error, not a rational one.
...
The book is unusually candid about both the mechanics and the emotional reality of trading a mechanical system, and its robustness statistics (RAR%, R-cubed, robust Sharpe) are a genuinely useful, if idiosyncratic, contribution not commonly taught elsewhere. Caveats: dollar and contract examples reflect mid-2000s futures pricing and account sizes and need rescaling for current markets; the alternative systems in Chapters 9–10 are illustrative and under-specified compared with the fully documented original Turtle rules; and Faith's claim that the original entry rules were unimportant to the Turtles' success sits in tension with the very detailed, parameter-specific rule set he reprints in full at the end of the book — readers should treat the "rules don't matter, discipline does" framing as a claim about his own experience, not as license to ignore parameter choices entirely.
