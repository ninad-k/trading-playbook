...
1. Identify a simple ABC correction against the established trend (Wave C exceeding Wave A's extreme).
2. Confirm Wave C is inside its typical WPT zone (Fibonacci-ratio projection).
3. Wait for a colored Reversal Bar (red = sell, blue = buy) to form inside that WPT zone.
4. Enter only once price breaks the Reversal Bar's low (short) or high (long).
5. Place the protective stop just beyond the Reversal Bar's opposite extreme (above the high for a short, below the low for a long).
6. Cap initial risk at roughly 2-3% of trading capital per trade; skip any set-up whose stop distance exceeds that cap.
...
Traders already familiar with basic technical analysis who want a rules-based (rather than purely discretionary) approach to Elliott wave trading, particularly the discipline of sizing trades by initial risk rather than target profit.
